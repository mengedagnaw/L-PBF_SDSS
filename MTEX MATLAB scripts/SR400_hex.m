%% SR400_hex.m  –  ODF φ2 sections for SR400 (hex EBSD, 0–90°×0–90°)
clearvars; close all; clc;

thisFile = mfilename('fullpath');
if ~isempty(thisFile), cd(fileparts(thisFile)); end
...
figFerr=figure('Name',condLabel+" – Ferrite ODF φ2");
plotSection(odfFerr,'phi2',phi2Vals); cb=mtexColorbar; set(get(cb,'Label'),'String','MRD');
formatODFAxes(win);
mtexTitle(condLabel+" hex – Ferrite ODF, \phi_2 = 0^\circ, 45^\circ, 65^\circ");
exportgraphics(figFerr, fullfile(outFig,"SR400_Ferrite_ODF_phi2_0_45_65.png"), 'Resolution', 600);

odfAust=calcDensity(ebsdAust.orientations,'halfwidth',hw,'resolution',res);
figAust=figure('Name',condLabel+" – Austenite ODF φ2");
plotSection(odfAust,'phi2',phi2Vals); cb=mtexColorbar; set(get(cb,'Label'),'String','MRD');
formatODFAxes(win);
mtexTitle(condLabel+" hex – Austenite ODF, \phi_2 = 0^\circ, 45^\circ, 65^\circ");
exportgraphics(figAust, fullfile(outFig,"SR400_Austenite_ODF_phi2_0_45_65.png"), 'Resolution', 600);

copyfile(thisFile + ".m", fullfile(outScr,"SR400_hex.m"));
disp("Done: " + condLabel);

function [ebsdFerr, ebsdAust] = pickDuplexPhases(ebsd)
ph = string(ebsd.mineralList);
ferr=""; aust="";
if any(strcmpi(ph,"Ferrite")), ferr="Ferrite"; end
if any(strcmpi(ph,"Austenite")), aust="Austenite"; end
if ferr==""
    idx=find(contains(lower(ph),"ferr"),1); if ~isempty(idx), ferr=ph(idx); end
end
if aust==""
    idx=find(contains(lower(ph),"aust")|contains(lower(ph),"gamma"),1);
    if ~isempty(idx), aust=ph(idx); end
end
if ferr=="" || aust==""
    error("Phase names not found. Phases: %s", strjoin(ph,", "));
end
ebsdFerr = ebsd(char(ferr));
ebsdAust = ebsd(char(aust));
end
